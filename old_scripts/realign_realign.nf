...
process realignGenomeBwameth {
		...
		export TMPDIR="$(pwd)/tmp"
                # reads_1 is actually a BAM and reads_2 is actually a BAI
                if [[ "!{reads_1}" = *.bam ]]; then
                    samtools collate -@ 2 -u -O !{reads_1} | samtools fastq -s /dev/null -0 /dev/null !{reads_1} | \
		    bwameth.py -p --reference !{ref} -t 16 - | samtools view -b - > !{reads_1.getFileName().toString().split("/")[-1].replace(".bam", "").replace(".fastq", "").replace(".fq", "")}_bwameth_!{suffix}.bam
                else
                    seqtk mergepe <(zcat -f !{reads_1}) <(zcat -f !{reads_2}) | \
		    bwameth.py --reference !{ref} -t 16 - | samtools view -b - > !{reads_1.getFileName().toString().split("/")[-1].replace(".bam", "").replace(".fastq", "").replace(".fq", "")}_bwameth_!{suffix}.bam
                fi
		'''
}

process indexBwameth {
...
process markDupsBwameth {
        ...
        penv 'smp'
        cpus 8
        conda 'picard==2.18.29'


        publishDir "${params.data_path}/bams_${suffix}_bismark", mode: 'copy'

        input:
                tuple file(sorted_bam), file(sorted_bam_bai)
        output:
                tuple file("*.sorted.md.bam"), file("*.sorted.md.bam.bai")
        ...
        shell:
                ...
                I=!{sorted_bam} \
                O="$bam_name_noext.md.bam" \
                M="$bam_name_noext_dup_metrics.txt";
                samtools index -@ 8 "$bam_name_noext.md.bam"
                '''
}
...
workflow {
  ...
  indexBwameth(realignGenomeBwameth.out)
  markDupsBwameth(indexBwameth.out)
